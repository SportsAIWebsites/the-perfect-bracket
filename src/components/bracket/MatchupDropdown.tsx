...
function generateMockOdds(topSeed: number, bottomSeed: number): GameOdds {
  ...
  let topML = spread <= 0 ? favML : dogML;
  let bottomML = spread <= 0 ? dogML : favML;
...
export function MatchupDropdown({
  ...
  const oddsQuery =
    !isDemo && topTeam && bottomTeam
      ? apiUrl(`/api/odds?home=${encodeURIComponent(topTeam.name)}&away=${encodeURIComponent(bottomTeam.name)}`)
  ...
  const { data: gameData } = useSWR<ESPNSummaryResponse>(
    !isDemo && matchup.espnEventId && (isFinal || isLive)
      ? apiUrl(`/api/game/${matchup.espnEventId}`)
      : null,
    fetcher,
